data "azurerm_image" "KPMDB01-Image" {
    name = "CustomImage"
...
resource "azurerm_virtual_machine" "KPMDB01" {
    ...
    storage_image_reference {
        id          = "${data.azurerm_image.KPMDB01-Image.id}"
    }

    storage_os_disk {
        name                = "${var.kpmdb01_name}_OS"
        caching             = "ReadWrite"
    ...
    os_profile {
        computer_name   = "${var.kpmdb01_name}"
